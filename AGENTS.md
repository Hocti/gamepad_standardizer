...
| `src/types.ts` | TypeScript types: `gamepadInfo`, `gamePadProfile`, `directSource` (+ PascalCase aliases `GamepadInfo`/`GamePadProfile`/`DirectionWrap`/`DpadPress`/`XY`) |
...
**Lazy DB contract:** the SDL DB is loaded only when a *non-standard* controller connects
(inside `getGamepadInfo`). Standard / xinput controllers never trigger a load (bundled import or
fetch). Keep it that way when editing `getGamepadInfo` / `ensureDB`.
